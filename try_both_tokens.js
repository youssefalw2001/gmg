const puppeteer = require('puppeteer-extra');
const StealthPlugin = require('puppeteer-extra-plugin-stealth');
puppeteer.use(StealthPlugin());
const fs = require('fs');

const FRESH_REFRESH = 'eyJhbGciOiJFUzI1NiIsInR5cCI6IkpXVCJ9..5G4VW1NtqC9JPF-gaEnIE3bjOp1KIA01ROcTEoIy7QxQzwKIJJy37Zit7ON5TrXyyCZFNSt2cHbbj6z1_jijMA';
const TARGET = '5KCVo4xnj8n38FUyF2gFUzgJbDW9vV4C1bTpciznyciU';
const PARAMS = 'device_id=de620a57-d98e-4293-b007-5d315455e21d&fp_did=4e8cc7f90484bd356291ff79db6b9b09&client_id=gmgn_web_20260715-2161-18d5b10&from_app=gmgn&app_ver=20260715-2161-18d5b10&tz_name=Asia%2FAden&tz_offset=10800&app_lang=en-US&os=web&worker=0';

const ANCHOR_TOKEN = process.env.ANCHOR_TOKEN || fs.readFileSync('/tmp/anchor_token.txt', 'utf8').trim();
const RELOAD_TOKEN = process.env.RELOAD_TOKEN || fs.readFileSync('/tmp/reload_token.txt', 'utf8').trim();

(async () => {
    console.log('🔥 TRYING BOTH TOKENS (ANCHOR + RELOAD)');
    console.log('Anchor:', ANCHOR_TOKEN.substring(0, 30) + '... len=' + ANCHOR_TOKEN.length);
    console.log('Reload:', RELOAD_TOKEN.substring(0, 30) + '... len=' + RELOAD_TOKEN.length);
    
    const browser = await puppeteer.launch({headless: 'new', args: ['--no-sandbox']});
    const page = await browser.newPage();
    await page.goto('https://gmgn.ai', {waitUntil: 'networkidle2', timeout: 25000});
    await new Promise(r => setTimeout(r, 3000));
    
    const result = await page.evaluate(async (refresh, params, target, anchorToken, reloadToken) => {
        const log = [];
        try {
            // Token
            let r = await fetch('/account/account/refresh_access_token', {
                method: 'POST', headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({refresh_token: refresh})
            });
            let text = await r.text();
            let data;
            try { data = JSON.parse(text); } catch(e) { return {error: 'non-json', text: text.substring(0,200)}; }
            if (data.code !== 0) return {error: 'token: ' + data.message};
            const token = data.data.data.token;
            const h = {'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token};
            log.push('✅ Token OK');
            
            // Generate txn_id
            r = await fetch('/account/account/generate_mfa_params?' + params, {
                method: 'POST', headers: h,
                body: JSON.stringify({usage: 'bind_wallet', biz_params: {address: target, wallet_type: 'sol', chain: 'sol'}})
            });
            data = await r.json();
            if (data.code !== 0) return {error: 'mfa: ' + data.message, log};
            const txnId = data.data.data.txn_id;
            log.push('✅ txn_id: ' + txnId);
            
            // fetch_captcha
            r = await fetch('/account/mfa/txn/v1/fetch_captcha?' + params, {
                method: 'POST', headers: h,
                body: JSON.stringify({usage: 'bind_wallet', txn_id: txnId})
            });
            data = await r.json();
            if (data.code !== 0) return {error: 'fetch_captcha: ' + data.message, log};
            const requestId = data.data.request_id;
            log.push('✅ request_id: ' + requestId);
            
            // Try ANCHOR token first
            log.push('--- Testing ANCHOR token ---');
            r = await fetch('/account/mfa/txn/v1/verify_captcha?' + params, {
                method: 'POST', headers: h,
                body: JSON.stringify({
                    txn_id: txnId, request_id: requestId, usage: 'bind_wallet',
                    captcha_type: 'recaptcha_score', captcha_data: anchorToken
                })
            });
            data = await r.json();
            log.push('Anchor: code=' + data.code + ' msg=' + (data.message||''));
            
            if (data.code === 0) {
                log.push('💀💀💀 ANCHOR TOKEN PASSED!!!');
                r = await fetch('/account/bind_wallet?' + params, {
                    method: 'POST', headers: h,
                    body: JSON.stringify({address: target, wallet_type: 'sol', chain: 'sol', txn_id: txnId})
                });
                data = await r.json();
                log.push('BIND: ' + JSON.stringify(data));
                return {log, SUCCESS: data.code === 0, method: 'anchor'};
            }
            
            // Try RELOAD token
            log.push('--- Testing RELOAD token ---');
            
            // Need fresh txn_id (old one might be used)
            r = await fetch('/account/account/generate_mfa_params?' + params, {
                method: 'POST', headers: h,
                body: JSON.stringify({usage: 'bind_wallet', biz_params: {address: target, wallet_type: 'sol', chain: 'sol'}})
            });
            data = await r.json();
            if (data.code !== 0) return {error: 'mfa2: ' + data.message, log};
            const txnId2 = data.data.data.txn_id;
            
            r = await fetch('/account/mfa/txn/v1/fetch_captcha?' + params, {
                method: 'POST', headers: h,
                body: JSON.stringify({usage: 'bind_wallet', txn_id: txnId2})
            });
            data = await r.json();
            if (data.code !== 0) return {error: 'fetch2: ' + data.message, log};
            const requestId2 = data.data.request_id;
            
            r = await fetch('/account/mfa/txn/v1/verify_captcha?' + params, {
                method: 'POST', headers: h,
                body: JSON.stringify({
                    txn_id: txnId2, request_id: requestId2, usage: 'bind_wallet',
                    captcha_type: 'recaptcha_score', captcha_data: reloadToken
                })
            });
            data = await r.json();
            log.push('Reload: code=' + data.code + ' msg=' + (data.message||''));
            
            if (data.code === 0) {
                log.push('💀💀💀 RELOAD TOKEN PASSED!!!');
                r = await fetch('/account/bind_wallet?' + params, {
                    method: 'POST', headers: h,
                    body: JSON.stringify({address: target, wallet_type: 'sol', chain: 'sol', txn_id: txnId2})
                });
                data = await r.json();
                log.push('BIND: ' + JSON.stringify(data));
                return {log, SUCCESS: data.code === 0, method: 'reload'};
            }
            
            return {log};
        } catch(e) { return {error: e.message, log}; }
    }, FRESH_REFRESH, PARAMS, TARGET, ANCHOR_TOKEN, RELOAD_TOKEN);
    
    console.log('\nRESULT:', JSON.stringify(result, null, 2));
    await browser.close();
})();
